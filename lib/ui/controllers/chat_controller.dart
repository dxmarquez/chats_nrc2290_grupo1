...
class ChatController extends GetxController {
  ...
  // dos usuarios
  Future<void> sendChat(senderUid, msg) async {
    AuthenticationController authenticationController = Get.find();
    String key = getChatKey(authenticationController.getUid(), senderUid);
    try {
      databaseReference
          .child('msg')
          .child(key)
          .push()
          .set({'senderUid': authenticationController.getUid(), 'msg': msg});
    } catch (error) {
      logError(error);
      return Future.error(error);
    }
